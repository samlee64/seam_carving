...
//EC
FloatImage gaussianAproxBlur(const FloatImage &im, int numBoxes, float sigma, float truncate=3.0, bool clamp=true);
FloatImage medianFilter(const FloatImage &im, int filterWidth, int filterHeight, bool clamp=true);
FloatImage fasterBoxBlur(const FloatImage &im, const int &k, bool clamp=true);
