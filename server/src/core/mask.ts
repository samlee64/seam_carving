...
export function createMask(params: CreateMaskParams, imageName: string, filename: string): string {
  ...
  const png = new PNG({ width: params.width, height: params.height, colorType: 0, inputColorType: 0 });
  for (let idx = 0; idx < data.length; idx++) {
    if (data[idx] != 0) png.data[idx] = 255;
  }

  const fullPath = path.join(config.dataDir, 'output', imageName, filename);
